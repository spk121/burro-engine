...
targa_error_t targa_parse_stream (GInputStream *istream, targa_image_t *t, gsize filesize)
{
    targa_error_t error;
    size_t id_size = 0;
...
guint8 *
tga_get_image_data_u8_ptr (const targa_image_t *t)
{
  return t->data.image_data_u8;
}

guint16 *
tga_get_color_map_data_u16_ptr (const targa_image_t *t)
{
...
targa_image_t *tga_load_from_resource (const gchar *resource)
{
  ...
  t = g_new0 (targa_image_t, 1);
  istream = xg_resources_open_stream (resource);
  filesize = xg_resources_get_filesize (resource);
  targa_parse_stream (istream, t, filesize);
  xg_input_stream_close (istream);
  return t;
}
