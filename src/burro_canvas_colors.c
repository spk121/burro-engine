...
#define COLOR_NAMES_COUNT 783
#define COLOR_NAMES_MAX_LEN 22

typedef struct color_name_tag {
    char name[23];
    uint32_t val;
} color_name_t;

color_name_t color_names[783] = {
    {"snow",                  0xfffafa},
...
burro_canvas_lookup_colorval (char *name, guint32 *val)
{
  ...
  for (int i = 0; i < COLOR_NAMES_COUNT; i ++)
    {
      if (strcasecmp (name, color_names[i].name) == 0)
	{
	  *val = color_names[i].val;
	  return TRUE;
	}
    }
