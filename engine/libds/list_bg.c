...
static void ds_obj_free (ds_bg_t *obj)
{
 free (obj);
}
#endif
...
{
  ds_bg_list_t *list = (ds_bg_list_t *) calloc (1, sizeof(ds_bg_list_t));
  if (list == NULL) {
			...
			size_t end_index, ds_bg_t *obj, ds_bg_node_t **N)
{
  ...
  *N = NULL;
  ...
  ds_bg_node_t *node = list->root.next;
...
{
  ds_bg_node_t *node2 = (ds_bg_node_t *) calloc (1, sizeof(ds_bg_node_t));
  *N = NULL;
  if (node2 == NULL) {
    err (ctx, "memory allocation failure");
    return DS_ERROR_OUT_OF_MEMORY;
		...
		ds_bg_node_t **N)
{
  ...
  node2 = (ds_bg_node_t *) calloc (1, sizeof (ds_bg_node_t));
  if (node2 == NULL) {
    err (ctx, "memory allocation failure");
    return DS_ERROR_OUT_OF_MEMORY;
  }
...
ds_bg_list_remove (ds_ctx_t *ctx, ds_bg_list_t *list, ds_bg_t *obj, bool *B)
{
  ...
  ret = ds_bg_list_search_from_to (ctx, list, 0, list->count, obj, &node);
  if (ret != DS_OK) {
    ...
    *B = false;
    ...
    ds_bg_list_remove_node (ctx, list, node);
    *B = true;
    return DS_OK;
  }
}
