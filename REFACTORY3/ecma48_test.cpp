...
{
    char do_bell[] = "\x07";
    ecma48_execute((const unsigned char *) do_bell, strlen(do_bell));
    
    char do_backspace[] = " \x08";
    ecma48_execute((const unsigned char *) do_backspace, strlen(do_backspace));

    char do_backtab[] = "\x1b" "[2Z";
    ecma48_execute((const unsigned char *) do_backtab, strlen(do_backtab));
}
